...
class LinSVM(SklearnModel):
    def __init__(self, model_args):
        super().__init__()
        # bp()
        self.classifier = SVC(C=model_args['C'], kernel=model_args['kernel'], class_weight=model_args['class_weight'],
                              verbose=model_args['verbose'], probability=model_args['probability'],
                              random_state=model_args['random_state'])
